...
1. 伪影出现时 `dmesg` 的 underrun 计数（桌面待机时为 0，叠加场景待测）
2. DDR/总线 devfreq 频率是否被降频（`/sys/class/devfreq/*`）
3. GPU busy 与合成器丢帧统计

候选修法：DDR/总线锁高频；重叠窗口场景自动回落 90 Hz；或接受为已知限制（游戏单 surface + timewarp 不受影响）。
